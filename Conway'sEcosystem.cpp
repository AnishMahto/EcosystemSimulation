...
//40 is the best of both worlds, imo
int width = 38;
int grid[38][38] = {0};
...
int whichAnimal (int y, int x) {
	...
	if (grid[y][x] == 1) {
		...
		eagles++;
		//eagle are capable of eating much more than 1 animal per gen, so we will set it to eat a max of 3. But, they must eat at least 1 to stay alive
		int eagleDigested = 0;
		for (int i = y - eagle.radius; i < y + eagle.radius && i < width; i++) {
			for (int z = x - eagle.radius; z < x + eagle.radius && z < width; z++) {
				...
				if ( (grid[i][z] == 6 || grid[i][z] == 5 || grid[i][z] == 4) && eagleDigested < 3) {
					grid[i][z] = 0;
					eagleDigested++;
					if (eagleDigested >= 1) {
						food = true;
